...
import { setCurPage } from "../../../features/projectFilter/projectFilterSlice";
...
const PaginationButtons = ({
  ...
  const { pageCount, curPage } = useSelector((store) => store.projectFilter);
  ...
  const selectedBtnClasses = "bg-black text-white border-black";
  ...
  return (
    <div className={`flex items-center gap-2 justify-center ${modifyClasses}`}>
      {curPage > 1 && (
        <button
      ...
      {buttons.length > 0 &&
        buttons.map((button, i, arr) => {
          if (arr.length > 3) {
            if (button.value > curPage - 2 && button.value < curPage + 2) {
              return (
                <button
                  ...
                  key={button.value}
                  onClick={() => {
                    dispatch(setCurPage(button.value));
                    scrollToTop();
                  }}
                >
                  {button.value}
